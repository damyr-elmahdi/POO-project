...
imgage=ImageTk.PhotoImage(Image.open("images/interfav.png"))
...
def add():
    ...
    def confirm():
        ...
        if image_path and price:
            # Calculate x and y based on the number of entries in the CSV
    ...
    def calculate_coordinates(csv_filename):
        try:
            with open(csv_filename, mode='r', newline='', encoding='utf-8') as file:
                reader = csv.reader(file)
                entries = list(reader)
                # Calculate the number of rows already present
                num_rows = len(entries) - 1  # Subtract 1 for the header
                x = (num_rows % 5) * 230 + 20  # Change 5 to how many books per row you want
                y = (num_rows // 5) * 230 + 100
                return x, y
        except FileNotFoundError:
            # If the file doesn't exist, start from the first position
            return 20, 100
...
booksdesc = {
    "book1": "Darkest before the dawn is a gripping tale of resilience, hope, and redemption in the face of adversity.",
    "book2": "Treasure Island is a classic adventure novel written by Robert Louis Stevenson, first published as a book in 1883.",
    "book3": "Magic of the Unicorn is a captivating fantasy novel that transports readers into a world filled with wonder, adventure, and enchantment.",
}
...
def modify_info():
    ...
    def update_price():
        ...
        # Update the book's price if the input is valid
    ...
    confirm_button = ctk.CTkButton(modify_info_window, text="Confirm", font=('Helvetica', 20), height=60, width=200, command=update_price and update_desc)
...
Add_b = customtkinter.CTkButton(win1, text="Add book ", font=('Avantstile Regular', 20),text_color="black", fg_color="blue", command=add)
